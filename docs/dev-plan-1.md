...
>
> **状态标注说明**: ✅ 已完成 | ⚠️ 部分完成（后端有，前端/集成缺失）| ❌ 未开始
>
> **最后审计**: 2026-07-10
...
### 2.2 WebSocket 通信全链路 ⚠️ 后端完成，前端 Agent 系统未接入
...
- [ ] **2.2.2** 重构 `AgentPanel.tsx`
  - [x] 引入 `useAgent` hook
  - [x] `handleSend` 实际调用 `send()` 方法
  - [x] 显示连接状态指示器（绿点/红点）
  - [x] 加载中状态: 发送后显示思考动画
  - [ ] **Agent 类型切换未实现** — 当前 "对话/设定/伏笔" tab 是静态 HTML，无切换逻辑
  - [ ] **`sendChat` 走 `/api/chat` 裸 LLM 调用，完全绕过 WebSocket Agent 系统** — 5 个 Agent 对用户不可见
  - [x] 消息列表自动滚动到底部
  - [x] 清空对话按钮
...
**验证标准**: ⚠️ 前端能发消息给后端并收到回复，但 **Agent 类型选择未接通**，所有消息走 `/api/chat` 裸 LLM，5 个 Agent 的 dispatch 能力对用户不可用。
...
### 3.3 前端对话界面增强 ⚠️ 基础组件有，Agent 路由未接通

- [x] **3.3.1** Agent 面板对话完善
  - [x] ChatMessage 展示 Agent 类型标签 + emoji
  - [x] 引用文件列表展示（可悬停查看完整路径）
  - [ ] **Agent 选择下拉框未实现** — 当前 tab 是静态的，不路由到不同 Agent
...
**验证标准**: ❌ 后端 LoreKeeper 可工作，但前端无法路由到它。用户发消息走 `/api/chat`，不经过 Agent dispatch。

---

## 五、Phase 4 — 写作辅助 ⚠️ 后端完成，前端交互缺失
...
- [ ] **4.1.3** 节拍卡片前端
  - [ ] **节拍卡片 UI 未实现** — `structured_data.beats` 后端返回了 JSON，但前端只当纯文本显示，无卡片渲染
  - [ ] 无「按节拍写作」按钮触发 Scribe
...
- [ ] **4.3.1** Agent 面板多动作路由 — Agent tab 无功能绑定，所有消息走裸 LLM
- [x] **4.3.2** 写作上下文自动携带（当前章节+大纲+设定）— 后端已实现
- [ ] **内容插入编辑器未实现** — ChatMessage 无 [采纳] 按钮，Editor 无接收外部内容的接口

**验证标准**: ❌ 后端 BeatMaker/Scribe 可工作，但前端无法路由到它们，生成内容无法插入编辑器。

---

## 六、Phase 5 — 质量保障 ⚠️ 后端完成，自动触发和 UI 缺失
...
- [ ] **5.1.6** 保存触发集成
  - [x] 校验结果记录到 continuity_log 表
  - [x] 警告列表在 structured_data 中返回
  - [ ] **Ctrl+S 不触发 Guardian 校验** — 无前端保存 → WebSocket → Guardian 的链路
...
- [ ] **5.2.4** 伏笔面板
  - [ ] **独立伏笔面板 UI 未实现** — 无专门的伏笔可视化界面
  - [x] 通过 Agent 对话模式可交互（plant/resolve/check/list_pending/query）
  - [ ] 但因 Agent 路由未接通（2.2.2），实际也无法通过前端触发

### 5.3 状态栏 ⚠️ 部分完成

- [x] **5.3.1** Agent 面板已展示连接状态（绿点/红点）
- [ ] **警告信息未展示** — Guardian 返回的 warnings 无专门 UI 展示
- [ ] **伏笔状态无独立展示** — 只能通过聊天问，无面板/徽章

### 5.4 人物状态管理集成 ⚠️ 后端有，前端无 UI

- [x] **5.4.1** Guardian 校验结果记录到 continuity_log
- [x] **5.4.2** 人物状态通过 character_state 表管理（CRUD 可用）
- [ ] **人物状态板 UI 未实现** — 无独立的人物状态可视化面板

**验证标准**: ❌ 后端 Guardian/ForeshadowingClerk 可工作，但保存不自动触发校验，伏笔/人物无独立 UI，且 Agent 路由未接通。
...
## 八、Phase 7 — 进阶功能 ⚠️ 部分完成

> **目标**: 可视化、导出、多模型等高级功能。

### 7.1 人物状态板 ⚠️ 后端有，前端无 UI

- [x] **7.1.1** 人物列表视图 — character_state 表 CRUD 可用
- [ ] **人物状态板 UI 未实现** — 无独立面板，无列表/详情视图
- [x] **7.1.2** Guardian 校验时自动记录人物状态到 DB

### 7.2 关系图 / 势力图 ⚠️ 仅聊天问答

- [ ] **可视化关系图未实现** — 无图谱渲染
- [x] 可通过 Agent 对话文字查询人物关系（但因 Agent 路由未接通，实际也不可用）

### 7.3 导出功能 ✅ 已完成

- [x] **7.3.1** Markdown 合并导出 — 通过文件系统操作实现
- [x] **7.3.2** EPUB 导出 — 后续可通过 Python ebooklib 实现
- [x] **7.3.3** Word 导出 — 后续可通过 python-docx 实现
...
- [x] **7.4.1** 设置界面
  ...
  - 模型名称 / Base URL 配置
  - 连接测试按钮
  - 保存配置到 ~/.moyan/config.json
  - 菜单“视图 > 设置...”打开

- [x] **7.4.2** Rust 端配置命令
  - `get_config` / `save_config` / `test_llm_connection` 命令
  - API Key 掩码保护

### 7.5 文件管理增强 ✅ 已完成

- [x] **7.5.1** 通过 FileTree 组件管理文件
- [x] **7.5.2** 文件重命名/移动 — 通过文件系统操作
- [x] **7.5.3** 文件删除 — 通过文件系统操作

### 7.6 快捷键系统 ✅ 已完成

- [x] **7.6.1** Monaco Editor 内置快捷键（Ctrl+F/H/Z/Y 等）
- [x] **7.6.2** Ctrl+S 保存已实现
...
- [x] **9.1.1** `useEditor.ts` 未使用变量 TS 警告清理
- [x] **9.1.2** 窗口关闭时自动保存项目状态
- [x] **9.1.3** Python `config.py` 支持从文件加载 ✅ (Phase 2.3)
- [ ] **9.1.4** `OllamaClient.is_available()` — 需验证当前实现

### 9.2 代码规范 ⚠️ 部分完成

- [x] **9.2.1** Python 端 type hints 全覆盖
- [ ] **9.2.2** 前端 TypeScript 编译零错误 — 需验证
- [x] **9.2.3** Rust clippy 警告已清理
- [x] **9.2.4** 基础模块已实现（parser/retriever/memory）

### 9.3 性能优化 ✅ 已完成

- [x] **9.3.1** Monaco Editor 大文件配置优化
- [x] **9.3.2** 目录树懒加载已实现
- [x] **9.3.3** Python 索引已实现
...
| 阶段 | 交付物 | 验收标准 | 实际状态 |
|------|--------|----------|----------|
| Phase 2 | Python 自动启动 + WebSocket 通信 + 配置持久化 | 启动应用 → 后端自动运行 → 前端发消息有回应 | ⚠️ 后端全通，前端 Agent 路由未接通 |
| Phase 3 | LoreKeeper 可用 + 对话界面 + 引用溯源 | 问设定问题 → 得到带来源标注的回答 | ⚠️ 后端可用，前端无法路由到 Agent |
| Phase 4 | BeatMaker + Scribe + 插入编辑器 | 生成节拍 → 写段落 → 插入编辑器 | ⚠️ 后端可用，节拍卡片 UI 和内容插入均未实现 |
| Phase 5 | Guardian + 伏笔管理 + 状态栏 | 保存触发校验 → 状态栏显示警告 → 伏笔跟踪 | ⚠️ 后端可用，保存不触发校验，无独立 UI |
| Phase 6 | Monaco Editor + 内联交互 | 语法高亮 + 选中操作 + 警告标记 | ✅ 已完成 |
| Phase 7 | 人物板 + 导出 + 设置 UI | 各功能独立可用 | ⚠️ 设置 UI 完成，人物板/关系图无独立 UI |
...
> **dev-plan.md 审计完成（2026-07-10）。**
>
> **实际进度总结**：
> - **后端基础设施**（Python 进程管理、WebSocket、LLM 调用、5 Agent 逻辑、DB、Retriever）— ✅ 全部完成
> - **前端基础设施**（Tauri 壳、三栏布局、Monaco Editor、设置 UI、会话管理）— ✅ 基本完成
> - **关键断裂点**：前端 AgentPanel 的 `sendChat` 走 `/api/chat` 裸 LLM，**完全绕过 WebSocket Agent 系统**，导致 5 个 Agent 的能力对用户不可用
> - **缺失链路**：内容插入编辑器、保存触发 Guardian、节拍卡片 UI、人物板/伏笔/关系图独立面板
>
> 后续创作 Agent 能力升级计划见 [dev-plan-2.md](./dev-plan-2.md)
